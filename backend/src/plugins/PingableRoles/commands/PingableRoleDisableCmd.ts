...
export const PingableRoleDisableCmd = pingableRolesCmd({
  ...
  async run({ message: msg, args, pluginData }) {
    ...
    if (!pingableRole) {
      void pluginData.state.common.sendErrorMessage(msg, `**${args.role.name}** is not set as pingable in <#${args.channelId}>`);
      return;
    }
    ...
    void pluginData.state.common.sendSuccessMessage(msg, `**${args.role.name}** is no longer set as pingable in <#${args.channelId}>`);
  },
});
